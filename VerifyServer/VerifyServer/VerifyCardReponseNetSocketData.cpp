...
CVerifyCardReponseNetSocketData::CVerifyCardReponseNetSocketData(int result, MyString strResult, MyString strCardNumber, MyString strCardPassword)
	: m_result(result)
...
CVerifyCardReponseNetSocketData::~CVerifyCardReponseNetSocketData(void)
{
}


int CVerifyCardReponseNetSocketData::_prepare_data(unsigned char* out_data, long len)
{
	int bret = false;
	...
	if(content!=0)
	{
		cJSON_AddStringToObject(content, JSON_COMMON_CTYPE_KEY, _T("card_verify_reponse"));
		...
		if(pjson_value!=0)
		{
			...
			cJSON_AddStringToObject(pjson_value, JSON_STRRESULT_KEY, m_strResult.c_str());
		}
	}
