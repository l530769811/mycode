...
const MyString CCheckInLoginInfoNetSocketDataParse::m_strRequest = _T("checkin_login_in");
CCheckInLoginInfoNetSocketDataParse::CCheckInLoginInfoNetSocketDataParse(CClientManager *mgr)
	: m_pgmr(mgr)
	, m_socketid(-1)
{
...
CCheckInLoginInfoNetSocketDataParse::~CCheckInLoginInfoNetSocketDataParse(void)
{
}


bool CCheckInLoginInfoNetSocketDataParse::_isType(const unsigned char* data, long len){
	bool bret = false;
...
bool CCheckInLoginInfoNetSocketDataParse::_parseData(unsigned long socket_id, const unsigned char* data, long len){
	bool bret = false;
 ...
 CUseCount<COperater> CCheckInLoginInfoNetSocketDataParse::CreateOperater()
 {
	 return CUseCount<COperater>( new CCheckInLoginInfoOperater(m_pgmr, m_socketid, m_strUserName, m_strUserPassword));
 }
