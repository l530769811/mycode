...
CConsumeCardReponseNetSocketData::CConsumeCardReponseNetSocketData(int result, MyString strCardNumber, MyString strCardPassword)
	: m_result(result)
...
CConsumeCardReponseNetSocketData::~CConsumeCardReponseNetSocketData(void)
{
}

int CConsumeCardReponseNetSocketData::_prepare_data(unsigned char* out_data, long len)
{
	int bret = false;
	...
	if(content!=0)
	{
		cJSON_AddStringToObject(content, JSON_COMMON_CTYPE_KEY, _T("consume_card_reponse"));
